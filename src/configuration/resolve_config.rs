...
pub fn resolve_config(config: ConfigKeyMap, global_config: &GlobalConfiguration) -> ResolveConfigurationResult<Configuration> {
  ...
  let resolved_config = Configuration {
    ...
    /* alignment */
    variable_statement_align_assignments: get_value(&mut config, "variableStatement.alignAssignments", false, &mut diagnostics),
    object_expression_align_properties: get_value(&mut config, "objectExpression.alignProperties", false, &mut diagnostics),
  };
